"""One-shot import of seo-llm Klorane personas + questions into a sen-ai scan.

Twin of import_seollm_ducray.py.

  - 5 source CSVs (bébé-klo, cheveux-sec-klo, chute-klo, pellicule-klo, pousse-klo)
  - 28 personas total, 418 questions total per local seo-llm cache snapshot

Run via:
    SCAN_ID=<the-klorane-scan-uuid> \
      SEOLLM_CACHE=/tmp/seollm_klorane \
      docker exec senai-worker python /tmp/import_seollm_klorane.py
"""
...
SEOLLM_CACHE = Path(os.environ.get("SEOLLM_CACHE", "/tmp/seollm_klorane"))

# seo-llm slug → sen-ai topic name prefix. The right-hand strings must match the
# topic names produced by align_topics_klorane.py (case-sensitive prefix match).
TOPIC_MAPPING = {
    "bébé-klo":        "Soin bébé",
    "cheveux-sec-klo": "Cheveux secs",
    "chute-klo":       "Chute",
    "pellicule-klo":   "Pellicules",
    "pousse-klo":      "Pousse",
}

# Local cache files are the _3 generation variant; per-file content holds 5-6
# personas regardless (the suffix is a generation seed, not a hard cap).
PERSONAS_PER_SOURCE = {slug: 3 for slug in TOPIC_MAPPING}

KLORANE_SOURCE_IDS = [5, 7, 9, 20, 21]
...
def find_topic(topics, prefix):
    # Filter is_active=True: an inactive topic with the same prefix must never
    # steal the match from the active target (Ducray prurit-duc footgun).
    for t in topics:
...
def main():
    ...
    for slug, topic in resolved.items():
        ...
        for p in personas:
            ...
            data["_source"] = {"origin": "seo-llm", "csv_slug": slug, "brand": "klorane"}
    ...
    cfg["import_source_ids"] = sorted(KLORANE_SOURCE_IDS)
    cfg["import_brand"] = "klorane"
    ...
    print(f"\n=========================")
    print(f"SUMMARY (Klorane import)")
