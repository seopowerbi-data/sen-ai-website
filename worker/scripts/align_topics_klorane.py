"""One-shot: align Klorane scan topic taxonomy to the 5 seo-llm source structure.

Twin of align_topics_ducray.py — adapted for Klorane's 5 source CSVs
(bébé, cheveux-sec, chute, pellicule, pousse) on klorane.com.

Klorane straddles two verticals: 4 hair-care topics + 1 baby-hygiene topic
(Klorane Bébé = bath / gentle cleansers for newborns, NOT hair).

Run via :
    SCAN_ID=<klorane-scan-uuid> docker exec senai-worker python /tmp/align_topics_klorane.py
"""
...
# Target 5 topics matching seo-llm Klorane sources.
TARGET_TOPICS = [
    {
        "key": "soin_bebe",
        "name": "Soin bébé et hygiène nourrisson",
        "description": "Bain et hygiène du nourrisson, nettoyants doux, liniment, change, soins lavants sans danger pour nouveau-nés et bébés — gamme Klorane Bébé.",
    },
    {
        "key": "cheveux_secs",
        "name": "Cheveux secs et abîmés",
        "description": "Cheveux secs, déshydratés, abîmés, fourchus, nutrition et réparation de la fibre, masques et après-shampoings nourrissants — gammes Mangue / Beurre de Cupuaçu.",
    },
    {
        "key": "chute",
        "name": "Chute de cheveux",
        "description": "Chute de cheveux saisonnière, réactionnelle, chronique, cheveux affaiblis, fortifiants et anti-chute capillaires — gamme Quinine.",
    },
    {
        "key": "pellicules",
        "name": "Pellicules et cuir chevelu",
        "description": "Pellicules sèches et grasses, démangeaisons du cuir chevelu, états squameux, soins assainissants et apaisants — gammes Ortie / Galanga.",
    },
    {
        "key": "pousse",
        "name": "Pousse et densité capillaire",
        "description": "Croissance des cheveux, densité, cheveux clairsemés, stimulation de la pousse, soins fortifiants et énergisants — gammes Quinine / Kératine végétale.",
    },
]
...
def main():
    ...
    def _matches(existing_name: str, key: str) -> bool:
        ...
        if key == "soin_bebe":
            return "bébé" in n or "bebe" in n or "nourrisson" in n or "nouveau-né" in n or "enfant" in n
        if key == "cheveux_secs":
            return "sec" in n or "abîm" in n or "abim" in n or "nutrition" in n or "réparation" in n
        if key == "chute":
            return "chute" in n or "alopéci" in n or "anti-chute" in n
        if key == "pellicules":
            return "pellicul" in n or "cuir chevelu" in n or "squam" in n or "démangeaison" in n
        if key == "pousse":
            return "pousse" in n or "densit" in n or "croissance" in n or "fortifi" in n
        return False
    ...
    print(f"\n=========================")
    print(f"SUMMARY (align_topics Klorane)")
